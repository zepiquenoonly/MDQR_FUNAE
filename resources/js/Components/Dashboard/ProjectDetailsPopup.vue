<template>
  ...
  >
    <div
      class="bg-white dark:bg-dark-secondary rounded-lg shadow-xl w-full max-w-6xl max-h-[98vh] overflow-hidden"
    >
      <!-- Header -->
      <div
      ...
      <!-- Content -->
      <div class="p-6 overflow-y-auto max-h-[calc(98vh-140px)]">
        <!-- Loading State -->
        ...
        <div v-else-if="!loading" class="text-center py-8">
          ...
          <button
            @click="loadProjectDetails(props.projectId)"
...
const saveProject = async () => {
  ...
  try {
    ...
    if (error.response?.data?.errors) {
      console.error(
        "Erro ao actualizar projecto: " + JSON.stringify(error.response.data.errors)
      );
    } else {
