[file name]: ProjectsManager.vue [file content begin]
<template>
  <div
    class="min-h-screen bg-gray-50 dark:bg-dark-primary p-2 sm:p-4 -mt-4 sm:-mt-6 relative"
  >
    ...
    <!-- CARDS SUPERIORES - Layout responsivo melhorado -->
    <div
      class="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 lg:gap-6 mb-4 sm:mb-8"
    >
      <!-- Card Finalizados -->
      <div
        class="bg-white dark:bg-dark-secondary rounded-lg sm:rounded-xl p-3 sm:p-6 shadow-sm hover:shadow-md transition-all duration-300"
      >
        <div class="flex items-center justify-between">
          <div class="min-w-0 flex-1">
            <p
              class="text-xs sm:text-sm font-bold text-green-700 dark:text-green-400 truncate"
            >
              Finalizados
            </p>
            <p
              class="text-lg sm:text-2xl lg:text-4xl font-bold text-gray-900 dark:text-dark-text-primary mt-1 sm:mt-2"
            >
              {{ stats.finished }}
            </p>
          ...
          <div
            class="w-6 h-6 sm:w-8 sm:h-8 lg:w-12 lg:h-12 bg-green-100 dark:bg-green-900/20 rounded-full flex items-center justify-center flex-shrink-0 ml-2"
          >
            <CheckCircleIcon
              class="w-3 h-3 sm:w-4 sm:h-4 lg:w-6 lg:h-6 text-green-600 dark:text-green-400"
            />
          </div>
        </div>
      </div>
      ...
      >
        <div class="flex items-center justify-between">
          <div class="min-w-0 flex-1">
            <p
              class="text-xs sm:text-sm font-bold text-yellow-700 dark:text-yellow-400 truncate"
            >
              Em Andamento
            </p>
            <p
          ...
          <div
            class="w-6 h-6 sm:w-8 sm:h-8 lg:w-12 lg:h-12 bg-yellow-100 dark:bg-yellow-900/20 rounded-full flex items-center justify-center flex-shrink-0 ml-2"
          >
            <ClockIcon
              class="w-3 h-3 sm:w-4 sm:h-4 lg:w-6 lg:h-6 text-yellow-600 dark:text-yellow-400"
            />
          </div>
        </div>
      </div>
      ...
      >
        <div class="flex items-center justify-between">
          <div class="min-w-0 flex-1">
            <p
              class="text-xs sm:text-sm font-bold text-red-700 dark:text-red-400 truncate"
            >
              Parados
            </p>
            <p
          ...
          <div
            class="w-6 h-6 sm:w-8 sm:h-8 lg:w-12 lg:h-12 bg-red-100 dark:bg-red-900/20 rounded-full flex items-center justify-center flex-shrink-0 ml-2"
          >
            <PauseCircleIcon
              class="w-3 h-3 sm:w-4 sm:h-4 lg:w-6 lg:h-6 text-red-600 dark:text-red-400"
            />
          </div>
        </div>
      </div>
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <div
            class="w-6 h-6 sm:w-8 sm:h-8 lg:w-12 lg:h-12 bg-blue-100 dark:bg-blue-900/20 rounded-full flex items-center justify-center flex-shrink-0 ml-2"
          >
            <FolderIcon
              class="w-3 h-3 sm:w-4 sm:h-4 lg:w-6 lg:h-6 text-blue-600 dark:text-blue-400"
            />
          </div>
        </div>
      </div>
    ...
    >
      ...
      >
        <h3
          class="text-base sm:text-lg font-semibold text-gray-800 dark:text-dark-text-primary"
        >
          Listagem de Projectos
        </h3>

        <!-- Pesquisa + Botão - Layout responsivo melhorado -->
        <div
          class="flex flex-col xs:flex-row gap-2 sm:gap-3 w-full sm:w-auto items-stretch"
        >
          <!-- Campo de pesquisa -->
      ...
      <div class="table-scroll-container">
              ...
              >
                ...
                <td class="py-2 px-1 sm:px-2 md:px-4">
                  <button
                    ...
                    class="bg-brand hover:bg-orange-600 text-white px-2 py-1 sm:px-3 sm:py-1.5 rounded text-xs font-semibold flex items-center gap-1 w-full justify-center whitespace-nowrap"
                  >
        ...
        <!-- Empty State -->
        <div v-else-if="projects.length === 0" class="text-center py-6 sm:py-8">
          <FolderIcon
          ...
          <button
            v-if="canEdit"
            @click="openRegisterForm"
            class="text-brand hover:text-orange-600 text-xs sm:text-sm font-medium mt-1 sm:mt-2"
          >
        ...
        <!-- No Results State -->
        <div
          v-else-if="filteredProjects.length === 0 && search"
