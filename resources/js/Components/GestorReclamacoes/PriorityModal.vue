<template>
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div class="bg-white dark:bg-dark-secondary rounded-xl shadow-lg max-w-md w-full p-6">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-dark-text-primary mb-4">Definir Prioridade</h3>
                ...
                <div class="space-y-2">
                    <button v-for="priority in priorities" :key="priority.value" @click="selectPriority(priority.value)"
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <span class="font-medium text-gray-900 dark:text-dark-text-primary">{{ priority.label
                                }}</span>
                        </div>
