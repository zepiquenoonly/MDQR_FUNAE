...
export const useIndicators = props => {
    ...
    const filteredIndicators = computed(() => {
        ...
        if (categoryFilter.value !== 'all') {
            indicators = indicators.filter(
                ind => ind.category === categoryFilter.value
            )
        }
    ...
    const summaryStats = computed(() => [
        ...
        {
            title: 'Tempo Médio',
            value: `${(props.grievanceStats.avg_resolution_time || 0).toFixed(
                1
            )} dias`,
    ...
    const quickStats = computed(() => [
        {
            ...
            value: props.grievanceStats.total || 0
        },
    ...
    const exportIndicators = async () => {
        ...
        try {
            // Usar o novo método que exporta TODOS os dados
            const response = await axios.get(
                '/gestor/indicators/export/pdf',
                {
