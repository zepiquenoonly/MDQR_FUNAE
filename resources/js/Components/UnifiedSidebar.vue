    ...
    >
      ...
      <div class="flex items-center space-x-3 min-w-0 relative z-10">
        ...
        <div class="flex items-center space-x-3 flex-1 min-w-0">
          <div
            class="w-12 h-12 bg-gradient-to-br from-primary-500 to-orange-600 rounded-xl flex items-center justify-center shadow-lg text-white flex-shrink-0 ring-2 ring-white/30 ring-offset-2 ring-offset-transparent transition-transform hover:scale-110 duration-300"
          >
            <img
              src="/images/Emblem_of_Mozambique.svg-2.png"
              alt="M"
          ...
          <div class="overflow-hidden min-w-0 flex-1">
            <h1
              class="font-bold text-lg whitespace-nowrap truncate text-gray-900 drop-shadow-sm"
            >
              MDQR
            </h1>
            <p class="text-primary-700 text-sm truncate font-medium">{{ roleLabel }}</p>
            <p v-if="user?.name" class="text-xs text-gray-600 truncate mt-1">
              {{ user.name }}
            </p>
          </div>
        </div>
