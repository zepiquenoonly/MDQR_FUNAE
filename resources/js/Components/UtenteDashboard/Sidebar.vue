<template>
  <aside :class="[
    ...
    <div class="p-5 flex items-center justify-between min-h-20">
      ...
      <button @click="$emit('toggle-sidebar')" class="text-white hover:text-gray-200 transition-colors">
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden">
      <MenuSection :is-collapsed="isCollapsed" @item-clicked="handleMenuItemClick" />
    </div>
  </aside>

  <!-- Overlay para mobile -->
  <div v-if="!isCollapsed && isMobile" class="fixed inset-0 bg-black bg-opacity-50 z-30 lg:hidden"
    @click="$emit('toggle-sidebar')"></div>

  <!-- Botão flutuante para abrir sidebar quando fechada em mobile -->
  <button v-if="isCollapsed && isMobile" @click="$emit('toggle-sidebar')"
