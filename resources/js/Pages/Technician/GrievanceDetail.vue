...
=======
    <Layout :role="'technician'">
        <div class="space-y-4 sm:space-y-6">
            <!-- Breadcrumb & Header -->
            <div class="flex flex-col gap-3 sm:gap-4">
                <Link href="/tecnico/dashboard" class="text-sm text-brand hover:text-orange-700 font-medium flex items-center p-2">
                    ← Voltar ao Painel
                </Link>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div class="lg:col-span-2 space-y-4 sm:space-y-6">
          ...
          >
            ...
            <div
              v-if="grievance.updates?.length === 0"
            ...
            <div v-else class="space-y-3 max-h-96 overflow-y-auto pr-2">
              <div
                v-for="(update, idx) in grievance.updates"
                :key="update.id"
                class="border-l-4 border-brand pl-4 pb-4"
                :class="{ 'border-gray-300 dark:border-gray-600': idx !== 0 }"
              >
                ...
                <p
                  v-if="update.comment"
                  class="text-sm text-gray-700 dark:text-gray-300 mt-2 italic bg-gray-50 dark:bg-dark-accent p-2 rounded"
                >
                  "{{ update.comment }}"
                </p>
                <div v-if="update.attachments?.length" class="mt-2 space-y-1">
